...
When you evaluate a session, cantoptek does not only grade the four fixed
...
```
cantoptek analyze / evaluate panel (explicit trigger)
...
The rubric lives inside the report JSON in `~/.cantoptek/reports`; there is
