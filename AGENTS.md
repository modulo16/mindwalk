...
`cantoptek` is a local visualizer for coding-agent sessions. It supports Claude Code, Codex, and pi, turning agent session logs plus repository structure into a deterministic 3D "code city" that can be explored in a browser.
...
- `cmd/cantoptek` provides the CLI commands: serve a local UI, open a session, build a citymap, export a trace, or evaluate a session.
